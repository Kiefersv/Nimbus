...
def picaso_formater(self, tag=None, ds_prev=None, path_to_opacities=None, sig=2,
    ...
    # ==== Check if optical properties should be calculated or loaded
    if opt_ds is None:
        # call optical properties function to calculate qext, qsca, and asym
        opts = optical_properties(
            self, tag=tag, ds_prev=ds_prev, path_to_opacities=path_to_opacities,
            sig=sig, mie_type=mie_type, nradii=nradii, mieai_object=mieai_object,
            time_stamps=time_stamps
        )
    else:
        # if a previous run is give, use that
        opts = opt_ds
    ...
    p_out = np.asarray([[i] * len(wave_in) for i in np.exp(self.logp_mid) * 1e-6])
    # wavelength of the output
    w_out = np.asarray([1 / wave_in / 1e-4] * len(self.logp_mid))
...
def optical_properties(self, tag=None, ds_prev=None, path_to_opacities=None, sig=2,
                       mie_type='full', nradii=100, mieai_object=None, time_stamps=None):
    """
    Create pandas dataframe of opacities that can be read into PICASO. This function is
    a simplified version of VIRGA code. Please cite Batalha et al. (2026) if you use it.
    ...
    sig : float
        Standard deviation for lognormal size radius distribution.
    mie_type : str, optional
        Can be 'full' for normal calculations, 'grid' for grid interpolation, 'ai' for
        an Ai model, or 'homogenous' if mie_net is not installed. Grids you need to
        produce yourself using MieNet, and only certain species are available within Ai
        models. 'homogenous' is selected if MieNet is not installed.
    nradii : int, optional
        Number of radius bins, 100 or more is recommended.
    mieai_object : MieAi class, optional
        Giving a mieai class rather than creating the default one. Allows for more
    ...
    if mieai_object is not None:
        ...
        use_ai = False
        if mie_type.upper() == 'AI':
            use_ai = True
        # try to load Mie Net
        try:
            from mienet import MieNet
            ma = MieNet(use_ai=use_ai)
        except ImportError:
            raise ValueError(
                'MieNet needs to be installed to use optical property calculations. '
                'Please go to https://github.com/d-attaway/MieNet'
            )
    ...
    opd = np.zeros((nt, nz, nw))  # total optical depth
    w0 = np.zeros((nt, nz, nw))  # single scattering albedo
    g0 = np.zeros((nt, nz, nw))  # asymmetry parameter
    ...
    for t, ti in enumerate(time_it):
        ...
        mask = scat_gas > 0
        opd[t, mask] = ext_gas[mask]
        w0[t, mask] = scat_gas[mask] / ext_gas[mask]
        g0[t, mask] = cqs_gas[mask] / scat_gas[mask]

    # ==== generate xarray output
    ds = xr.Dataset(
        data_vars={
            'opd': (['time', 'pressure', 'wavelength'], opd),
            'w0': (['time', 'pressure', 'wavelength'], w0),
            'g0': (['time', 'pressure', 'wavelength'], g0),
        },
        coords={
            'time': time_it,
            'pressure': self.pres,
            'wavelength': 1 / wave_in / 1e-4,
        },
    )
    return ds
